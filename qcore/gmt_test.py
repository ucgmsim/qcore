...
def test_cpt(pf):
    ...
    gmt.makecpt("hot", cptf, 0, 120, inc=0.1, invert=True, wd=os.path.dirname(pf))
    p = gmt.GMTPlot(pf)
...
def run_test(test, gmt_version):
    ...
    if test[1] is None:
        if success:
            ...
            print(
                "%s [%s] FAIL (%s) %.2fs" % (gmt_version, test[0].__name__, message, t)
            )
        return success
    ...
    try:
        ...
        os.symlink(pp, os.path.join(test_dir, test[0].__name__, os.path.basename(pp)))
    except IOError:
        print("%s [%s] FAIL (NO OUTPUT) %.2fs" % (gmt_version, test[0].__name__, t))
        return False

    if ph == test[1]:
        print("%s [%s] PASS %.2fs" % (gmt_version, test[0].__name__, t))
        return True
    print("%s [%s] FAIL (%s) %.2fs" % (gmt_version, test[0].__name__, ph, t))
    return False
...
if rank == MASTER:
    ...
    while workers_closed < workers:
        ...
        if tag == tags.READY:
            found_job = False
            while job < jobs:
                workload = (TESTS[job % len(TESTS)], gmt_versions[job // len(TESTS)])
