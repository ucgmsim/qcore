...
SRF_1_PATH = os.path.join(
    ...
    "sample1/input/Hossack_HYP01-01_S1244.srf",
)
SRF_2_PATH = os.path.join(
    os.path.abspath(os.path.dirname(__file__)),
    "sample2/input/Tuakana13_HYP01-01_S1244.srf",
)
SRF_3_PATH = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "sample3/input/single_point_source.srf"
)  # This is a fake one, just created for testing single point source
SRF_1_CNR_PATH = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "sample1/output/cnrs.txt"
)
SRF_2_CNR_PATH = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "sample2/output/cnrs.txt"
)
SRF_1_OUT_ARRAY_SRF2LLV = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "sample1/output/out_array_srf2llv.bin"
)
SRF_2_OUT_ARRAY_SRF2LLV = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "sample2/output/out_array_srf2llv.bin"
)
SRF_1_OUT_ARRAY_SRF2LLV_PY = os.path.join(
    os.path.abspath(os.path.dirname(__file__)),
    "sample1/output/out_array_srf2llv_py.bin",
)
SRF_2_OUT_ARRAY_SRF2LLV_PY = os.path.join(
    os.path.abspath(os.path.dirname(__file__)),
    "sample2/output/out_array_srf2llv_py.bin",
)
...
@pytest.mark.parametrize(
    "plane, expected_values",
    [
        (
            SRF_1_PLANES[0],
            [[176.2354, -38.3404], 34, 92, 3.44, 9.24, 230, 60, 0.00, 0.00, 5.54],
        ),
        (
            SRF_2_PLANES[0],
            [[176.8003, -37.0990], 46, 104, 4.57, 10.44, 21, 50, 0.00, 0.00, 6.27],
        ),
        (
            SRF_2_PLANES[1],
            [
...
@pytest.mark.parametrize(
    "test_dt, expected_dt", [(SRF_1_PATH, 2.50000e-02), (SRF_2_PATH, 2.50000e-02)]
)
...
@pytest.mark.parametrize(
    "test_dxy, expected_dxy", [(SRF_1_PATH, (0.10, 0.10)), (SRF_2_PATH, (0.1, 0.10))]
)
...
@pytest.mark.parametrize(
    "test_srf,filename,sample_cnr_file_path",
    [
        (SRF_1_PATH, "cnrs1.txt", SRF_1_CNR_PATH),
...
@pytest.mark.parametrize(
    "test_srf,expected_latlondepth",
    [
        (SRF_1_PATH, {"lat": -38.3354, "depth": 0.0431, "lon": 176.2414}),
        (SRF_2_PATH, {"lat": -37.1105, "depth": 0.0381, "lon": 176.7958}),
    ],
)
def test_read_latlondepth(
    test_srf, expected_latlondepth
):  # give you so many lat,lon,depth points
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [
                [
                    (176.7922, -37.118, 0.0381),
                    (176.8101, -37.0806, 0.0381),
                    (176.876, -37.1089, 7.8931),
                    (176.8581, -37.1464, 7.8931),
                ],
                [
                    (176.8107, -37.0798, 0.038),
                    (176.8433, -37.0455, 0.038),
                    (176.9092, -37.0739, 7.8672),
                    (176.8765, -37.1082, 7.8672),
                ],
            ],
        ),
        (
...
@pytest.mark.parametrize(
    "test_srf, expected_result", [(SRF_1_PATH, True), (SRF_2_PATH, True)]
)
...
@pytest.mark.parametrize(
    "test_srf_planes, expected_result", [(SRF_1_PLANES, 1), (SRF_2_PLANES, 2)]
)
...
@pytest.mark.parametrize(
    "test_srf, expected_result",
    [
        (SRF_1_PATH, (AssertionError)),
        (SRF_2_PATH, AssertionError),
        (SRF_3_PATH, (0, 60, 30)),
    ],
)
def test_ps_params(test_srf, expected_result):
    try:
        ...
        return
    assert (
        srf.ps_params(test_srf) == expected_result
