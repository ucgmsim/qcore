...
TMP_DIR_NAME = (os.path.join("/home/", getpass.getuser(), (
"tmp_" + os.path.basename(__file__)[:-3] + '_' + ''.join(str(datetime.now()).split())).replace('.', '_')).replace(
    ':', '_'))
...
@pytest.mark.parametrize("gmt_format, expected_corners", [(False, [[170.9143829345703, -43.291595458984375],
                                                                   [172.26246643066406, -44.217742919921875],
                                                                   [171.01295471191406, -45.15299987792969],
                                                                   [169.66343688964844, -44.21214294433594]]), \
                                                          (True, ([[170.9143829345703, -43.291595458984375],
                                                                   [172.26246643066406, -44.217742919921875],
                                                                   [171.01295471191406, -45.15299987792969],
                                                                   [169.66343688964844, -44.21214294433594]],
...
@pytest.mark.parametrize("corners, expected_region",
                         [(None, (169.66343688964844, 172.26246643066406, -45.15299987792969, -43.291595458984375)), \
                          ([[170.91449, -43.2916], [172.26257, -44.21773], [171.01305, -45.15298],
                            [169.66354, -44.21213]],
                           (169.66354000000001, 172.26257000000001, -45.152979999999999, -43.291600000000003))])
def test_region(corners, expected_region):
    assert OBJ_XYTS.region(corners) == expected_region


@pytest.mark.parametrize("mmi, pgvout, mmiout, sample_pgv, sample_mmi",
                         [(True, None, None, SAMPLE_PGV, SAMPLE_MMI), (False, None, None, SAMPLE_PGV, SAMPLE_MMI), \
