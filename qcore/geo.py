...
def xy2ll(xy_km: np.ndarray, amat: np.ndarray) -> np.ndarray:
    ...
    y = xy_km[:, 1] / R_EARTH
    sin_g = np.sin(y)
    z = np.sqrt(1.0 + sin_b * sin_b * sin_g * sin_g)
    xp = sin_g * np.cos(x) * z
    yp = sin_b * np.cos(y) * z
...
def ll_mid(lon1: float, lat1: float, lon2: float, lat2: float) -> tuple[float, float]:
    ...
    """
    # functions based on np.radians
    lon1, lat1, lat2, dlon = list(map(np.radians, [lon1, lat1, lat2, (lon2 - lon1)]))
...
def ll_dist(lon1: float, lat1: float, lon2: float, lat2: float) -> float:
    ...
    lat1, lat2, dlon, dlat = list(
        map(np.radians, [lat1, lat2, (lon2 - lon1), (lat2 - lat1)])
    )
...
def rotation_matrix(angle: float) -> np.ndarray:
    ...
    ----------
    angle : float
        The angle to rotate by in np.radians.
...
def ll_cross_along_track_dist(
    ...
    """
    if a12 is None:
        a12 = np.radians(ll_bearing(lon1, lat1, lon2, lat2))
    if a13 is None:
        a13 = np.radians(ll_bearing(lon1, lat1, lon3, lat3))
    if d13 is None:
